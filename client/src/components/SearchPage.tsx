...
import { Guide } from "@/types/guideType";
...
const SearchPage = () => {
  ...
  useEffect(() => {
    searchGuide(params.text!, searchQuery, appliedFilter);
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            <Input
              type="text"
              value={searchQuery}
              placeholder="Search by guide & services"
          ...
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
              <h1 className="font-medium text-lg">
                ({searchedGuide?.data.length}) Search result found
              </h1>
              <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
                {appliedFilter.map((selectedFilter: string, idx: number) => (
                  ...
                  >
                    <Badge
                      className="text-[#7d239b] rounded-md hover:cursor-pointer pr-6 whitespace-nowrap"
                      variant="outline"
                    >
                      {selectedFilter}
                    </Badge>
                    <X
                      ...
                      size={16}
                      className="absolute text-[#7d239b] right-1 hover:cursor-pointer"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {loading ? (
                <SearchPageSkeleton />
              ) : !loading && searchedGuide?.data.length === 0 ? (
                <NoResultFound searchText={params.text!} />
              ) : (
                searchedGuide?.data.map((guide: Guide) => (
                  <div className="group transition-transform duration-300 hover:scale-105">
                    <Card
                      key={guide._id}
                      className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden hover:shadow-2xl"
                    >
...
const NoResultFound = ({ searchText }: { searchText: string }) => {
  return (
    <div className="text-center">
      ...
      <Link to="/">
        <Button className="mt-4 bg-purple hover:bg-hoverPurple">
          Go Back to Home
        </Button>
      </Link>
    </div>
